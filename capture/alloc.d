...
alias Allocator0 = Mallocator;
alias Allocator0s = StatsCollector!(Allocator0, Options.all);
__gshared Allocator0s* allocator0s;

struct Allocator0sG
{
	// static Allocator0s* instance() { return allocator0s; }
	alias allocator0s instance;
	alias instance this;
}

//alias Allocator1 = Region!(Allocator0s*);
alias Allocator1 = Region!(Allocator0sG);
alias Allocator = AllocatorList!((n)
	// {
	// 	auto size = max(n, 1024 * 4096);
	// 	auto parent = allocator0s;
	// 	auto buf = cast(ubyte[])(parent.allocate(size.roundUpToAlignment(platformAlignment)));
	// 	auto a = Allocator1(buf);
	// 	a.parent = parent;
	// 	return a;
	// }
	=> Allocator1(max(n, 1024 * 4096))
	, Allocator0sG);
__gshared Allocator* allocator;
//__gshared Allocator0s* allocatorStats;
...
private size_t roundUpToAlignment(size_t n, uint alignment)
{
...
public:
    ...
    */
	...
	{
		import std.stdio;
		reportStatistics(stdout);
	}

	~this() @nogc
	{
		(cast(void delegate() @nogc)&dump)();
	}
}
